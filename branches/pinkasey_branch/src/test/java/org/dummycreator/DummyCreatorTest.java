...
import org.dummycreator.helperutils.*;
...
import org.dummycreator.helperutils.NestedMapClass.*;
...
public class DummyCreatorTest {
    ...
    public void testIntRange(){
        int min = 0;
        int max = 100;
        DummyCreator myCreator = new DummyCreator(ClassBindings.defaultBindings().add(Integer.class, new RangeFactory<Integer>(min,max)));
        for(int i=0; i<10000; i++){
//            MyClass a = myCreator.create(MyClass.class);
            Integer a = myCreator.create(Integer.class);
            System.out.println(a);
            Assert.assertTrue(a >= min);
            Assert.assertTrue(a < max);
        }
    }

    @Test
    public void testFillNonProperties(){
        testFillNonProperties(true, null, true);
        testFillNonProperties(true, true, true);
        testFillNonProperties(true, false, false);
        testFillNonProperties(false, null, false);
        testFillNonProperties(false, true, true);
        testFillNonProperties(false, false, false);
    }

    public void testFillNonProperties(final boolean classBindFill, final Boolean factoryFill, final boolean shouldFill){
        DummyCreator creator = new DummyCreator(
                ClassBindings.defaultBindings()
                        .setFillNonPropertyFields(classBindFill)
                        .add(MyClass.class,
                                new ClassBasedFactory<MyClass>(MyClass.class)
                                        .setIsFillNonPropertyFields(factoryFill)
                        )
        );

        MyClass instance = creator.create(MyClass.class);

        if(shouldFill){
            Assert.assertNotNull(instance.getPrivateInt());
            Assert.assertNotNull(instance.getPrivateInt());
        } else {
            Assert.assertNull(instance.getPrivateInt());
            Assert.assertNull(instance.getPrivateInt());
        }
    }
    ...
    public void testSpecificField(){
        testSpecificField("memberInt", new RangeFactory<Integer>(1,2), 1);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testSpecificFieldNoField(){
        testSpecificField("memberInt111", new RangeFactory<Integer>(1,2), null);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testSpecificFieldBadFactory(){
        testSpecificField("memberInt", new ClassBasedFactory<String>(String.class), null);
    }

    private <T> void testSpecificField(String fieldName, DummyFactory<T> factory, Integer expected){
        DummyCreator creator = new DummyCreator(
                ClassBindings.defaultBindings()
                        .add(MyClass.class,
                                new ClassBasedFactory<MyClass>(MyClass.class)
                                        .setField(fieldName, factory)
                        )
        );

        MyClass instance = creator.create(MyClass.class);
        Assert.assertEquals(expected, instance.getMemberInt() );
    }
     ...
     * bindings if a <code>LinkedList</code> is now re instead.
     */
    @Test
    ...
    public void testArrayCreation() {
        final Integer[] integers = dummyCreator.create(Integer[].class);
        assertNotNull(integers);
        for (final Integer i : integers) {
            assertSame(Integer.class, i.getClass());
        }
        final int[] ints = dummyCreator.create(int[].class);
        assertNotNull(ints);
    }
    ...
    public void testDummyCreatorBigDecimal() {
        ...
        assertTrue(new BigDecimal(55.5).equals(bigDecimal) || new BigDecimal(33686018).equals(bigDecimal));
    ...
    @Test(expected = IllegalArgumentException.class)
    public void testInterfaceBindingErrors2() throws Exception {
        dummyCreator = new DummyCreator();
